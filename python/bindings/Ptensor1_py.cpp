pybind11::class_<Ptensor1<float>>(m,"ptensor1")
  ...
  .def_static("view",[](at::Tensor& x, const vector<int>& v){return Ptensor1<float>::view(x,v);})
  .def("torch",[](const Ptensor1<float>& x){return x.torch();})

  .def("add_linmaps",[](Ptensor1<float>& obj, const Pten0& x, int offs){
      return obj.add_linmaps(x,offs);}, py::arg("x"), py::arg("offs")=0)
  .def("add_linmaps",[](Ptensor1<float>& obj, const Ptensor1<float>& x, int offs){
      return obj.add_linmaps(x,offs);}, py::arg("x"), py::arg("offs")=0)

  .def("add_linmaps_to",[](const Ptensor1<float>& obj, Pten0& x, int offs){

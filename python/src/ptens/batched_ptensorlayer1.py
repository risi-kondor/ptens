...
class batched_ptensorlayer1(p.batched_ptensorlayer):
    ...
    def gather(self,atoms,x,*args):
        ...
        if len(args)==0:
            return batched_ptensorlayer1.gather(atoms,x,pb.batched_ptensor_map.overlaps(atoms,x.atoms)) 
        else:
    ...
    def __repr__(self):
        return "batched_ptensorlayer1(len="+str(len(self))+",nc="+str(self.size(1))+")"
...
class batched_ptensorlayer1_gatherFn(torch.autograd.Function):
    ...
    def forward(ctx,atoms,x,tmap):
        r=batched_ptensorlayer1.zeros(atoms,x.get_nc()*([1,2,5][x.getk()]))

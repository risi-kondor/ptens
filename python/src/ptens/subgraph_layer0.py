...
class SubgraphLayer0_toMxFn(torch.autograd.Function):
    ...
    def backward(ctx,g):
       R=subgraph_layer0(1)
       ctx.x.add_to_grad(_subgraph_layer0(g))
       return R
...
class Subgraph_layer0_toFn(torch.autograd.Function):
...
class Subgraph_layer0_mprodFn(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx,x,y):
        R=ptens.subgraph_layer0.zeros(x.obj.view_of_atoms(),y.size(1),x.obj.get_dev())
    ...
    def backward(ctx,g):
        ...
        return subgraph_layer0.dummy(), ctx.x.mprod_back1(ctx.r.gradp())
...
class Subgraph_layer0_linearFn(torch.autograd.Function):
    ...
    def backward(ctx,g):
        ...
        return subgraph_layer0.dummy(), ctx.x.linear_back1(ctx.r.gradp()), ctx.x.linear_back2(ctx.r.gradp())
...
class Subgraph_layer0_Transfer0Fn(torch.autograd.Function):

    @staticmethod
    def forward(ctx,x,atoms,G):
        ctx.x=x
        R=ptens.subgraph_layer0.zeros(atoms,x.obj.get_nc(),x.obj.get_dev())
...
class Subgraph_layer0_Transfer1Fn(torch.autograd.Function):
...
class Subgraph_layer0_Transfer2Fn(torch.autograd.Function):
...
class Subgraph_layer0_Unite1Fn(torch.autograd.Function):
...
class Subgraph_layer0_Unite2Fn(torch.autograd.Function):
...
class Subgraph_layer0_GatherFn(torch.autograd.Function):

    @staticmethod
    def forward(ctx,x,G):
        r=ptens.subgraph_layer0(1)
...
class Subgraph_layer0_Outer1Fn(torch.autograd.Function):
    ...
    def backward(ctx,g):
        ptens_base.add_outer_back0(ctx.x.gradp(),ctx.r.gradp(),ctx.y)
        ptens_base.add_outer_back1(ctx.y.gradp(),ctx.r.gradp(),ctx.x)
        return subgraph_layer0.dummy(), ptens.ptensors1.dummy()
...
class Subgraph_layer0_Outer2Fn(torch.autograd.Function):
    ...
    def backward(ctx,g):
        ptens_base.add_outer_back0(ctx.x.gradp(),ctx.r.gradp(),ctx.y)
        ptens_base.add_outer_back1(ctx.y.gradp(),ctx.r.gradp(),ctx.x)
        return subgraph_layer0.dummy(), ptens.ptensors2.dummy()

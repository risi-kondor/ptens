...
import ptens_base
from ptens_base import batched_ptensors2b as _batched_ptensors2b
...
class batched_ptensors2b(torch.Tensor):

    @classmethod
    def dummy(self):
        R=batched_ptensors2b(1)
        return R
    ...
    def from_matrix(self,M,atoms):
        return Batched_ptensors2b_fromMxFn.apply(M,atoms)
    ...
    def zeros(self, _atoms, _nc, device='cpu'):
        R=batched_ptensors2b(1)
        R.obj=_batched_ptensors2b.create(_atoms,_nc,0,device_id(device))
        return R

    @classmethod
    def randn(self, _atoms, _nc, device='cpu'):
        R=batched_ptensors2b(1)
        R.obj=_batched_ptensors2b.create(_atoms,_nc,4,device_id(device))
        return R

    @classmethod
    def sequential(self, _atoms, _nc, device='cpu'):
        R=batched_ptensors2b(1)
        R.obj=_batched_ptensors2b.create(_atoms,_nc,3,device_id(device))
        return R

    def randn_like(self):
        return batched_ptensors2b.init(self.obj.randn_like())
    
    @classmethod
    def cat(self,*args):
        return BatchedPtensors2b_catFn.apply(self,*args)
    ...
    def outer(self,y):
         return Batched_ptensors2b_outerFn.apply(self,y)
    ...
    @classmethod
    def gather(self,x,S):
        return BatchedPtensors2b_GatherFn.apply(x,S)
...
class Batched_ptensors2b_fromMxFn(torch.autograd.Function):

    @staticmethod
    def forward(ctx,x,atoms=None):
        R=batched_ptensors2b(1)
        if atoms is None:
            R.obj=_batched_ptensors2b(x)
        else:
            R.obj=_batched_ptensors2b(atoms,x)
        ctx.r=R.obj
        return R
...
class Batched_ptensors2b_catFn(torch.autograd.Function):
    
    @staticmethod
    def forward(ctx,dummy,*args):
        r=batched_ptensors2b.dummy()
        ctx.args=[x.obj for x in args]
        r.obj=_batched_ptensors2b.cat(ctx.args)
        ctx.r=r.obj
        return r
    ...
    def backward(ctx,g):
        ...
        for x in ctx.args:
            ...
            offs=offs+x.dim(0)
            dummies.append(batched_ptensors2b.dummy())
        return None, *dummies #it was *dummies


class Batched_ptensors2b_outerFn(torch.autograd.Function):
    ...
    def backward(ctx,g):
        ...
        return batched_ptensors2b.dummy(), batched_ptensors2b.dummy()


class BatchedPtensors2b_LinmapsFn(torch.autograd.Function):

    @staticmethod
    def forward(ctx,x):
        r=x.dummy()
        r.obj=_batched_ptensors2b.linmaps(x.obj) 
        ctx.x=x.obj
        ctx.r=r.obj
        return r
...
class BatchedPtensors2b_GatherFn(torch.autograd.Function):

    @staticmethod
    def forward(ctx,x,atoms):
        r=x.dummy()
        r.obj=_batched_ptensors2b.gather(x.obj,atoms)
        ctx.x=x.obj
        ctx.r=r.obj
        return r

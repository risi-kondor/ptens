...
class subgraph_layer2(torch.Tensor):

    @classmethod
    def from_matrix(self,G,S,atoms,M):
        return SubgraphLayer2_fromMxFn.apply(G,S,atoms,M)

    @classmethod
    def dummy(self):
        R=subgraph_layer2(1)
        R.obj=_subgraph_layer2.dummy()
        return R
    ...
    def randn_like(self,sigma=1.0):
        return subgraph_layer2.randn(self.get_atoms(),self.get_nc(),sigma,self.get_dev())

    def torch(self):
        return Subgraph_layer2_toMxFn.apply(self)

    def to(self, device='cpu'):
        return Subgraph_layer2_toFn.apply(self,device)
    ...
    def outer(self,y):
        ...
        if isinstance(y,ptens.subgraph_layer2):
    ...
    def unite1(self,G,normalized=False):
        return Subgraph_layer2_Unite1Fn.apply(self,G,normalized)
    
    def unite2(self,G,normalized=False):
        return Subgraph_layer2_Unite2Fn.apply(self,G,normalized)
...
class Subgraph_layer2_fromMxFn(torch.autograd.Function):

    @staticmethod
    def forward(ctx,x,atoms):
        R=subgraph_layer2(1)
        R.obj=_subgraph_layer2(x,atoms)
        ctx.r=R.obj
        return R
...
class Subgraph_layer2_toMxFn(torch.autograd.Function):
    ...
    def backward(ctx,g):
       R=subgraph_layer2(1)
       ctx.x.add_to_grad(_subgraph_layer2(g,ctx.x.get_atoms()))
       return R
...
class Subgraph_layer2_addFn(torch.autograd.Function):
    ...
    def backward(ctx,g):
        ...
        return subgraph_layer2(1),subgraph_layer2(1)
...
class Subgraph_layer2_ReLUFn(torch.autograd.Function):
    ...
    def forward(ctx,x,alpha):
        R=ptens.subgraph_layer2.zeros(x.obj.view_of_atoms(),x.obj.get_nc(),x.obj.get_dev())
...
class Subgraph_layer2_mprodFn(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx,x,y):
        R=ptens.subgraph_layer2.zeros(x.obj.view_of_atoms(),y.size(1),x.obj.get_dev())
    ...
    def backward(ctx,g):
        ...
        return subgraph_layer2(1), ctx.x.mprod_back1(ctx.r.gradp())
...
class Subgraph_layer2_Linmaps0Fn(torch.autograd.Function):
    ...
    def backward(ctx,g):
        ...
        return subgraph_layer2(1), None


class Subgraph_layer2_Linmaps1Fn(torch.autograd.Function):

    @staticmethod
    def forward(ctx,x,normalized=False):
        R=ptens.subgraph_layer2.zeros(x.obj.view_of_atoms(),x.obj.get_nc()*2,x.obj.get_dev())
...
class Subgraph_layer2_Linmaps2Fn(torch.autograd.Function):
...
class Subgraph_layer2_Outer0Fn(torch.autograd.Function):
    ...
    def backward(ctx,g):
        ...
        return subgraph_layer2.dummy(), ptens.ptensors0.dummy()


class Subgraph_layer2_Outer1Fn(torch.autograd.Function):

...
class TestPtensors2b(object):

    def backprop(self,fn,_atoms,_nc):
        x=p.ptensors2b.randn(_atoms,_nc)
        x.requires_grad_()
        z=fn(x)
        ...
        xeps=p.ptensors2b.randn(_atoms,_nc)
        z=fn(x+xeps)
        xloss=z.inp(testvec)
    ...
    def backprop2(self,fn,_atoms1,_atoms2,_nc):
        x=p.ptensors2b.randn(_atoms1,_nc)
        x.requires_grad_()
        z=fn(x,_atoms2)
        ...
        xeps=p.ptensors2b.randn(_atoms1,_nc)
        z=fn(x+xeps,_atoms2)
        xloss=z.inp(testvec)
        assert(torch.allclose(xloss-loss,xeps.inp(xgrad),rtol=1e-3, atol=1e-4))

...
namespace ptens{
  ...
  public:
    ...
    shared_ptr<AindexPack> in_map;
    shared_ptr<AindexPack> out_map;
    ...
    TransferMap(const ATOMSPACK& _in_atoms, const ATOMSPACK& _out_atoms):
      ...
      in_atoms(&_in_atoms), 
      out_atoms(&_in_atoms),
      in_map(new AindexPack()),
      out_map(new AindexPack()){
      make_overlaps();
      make_intersects();
    }
  ...
  public: // ---- Overlaps -----------------------------------------------------------------------------------
    ...
    void make_overlaps(){
      cnine::flog timer("TransferMap::make_overlaps");
      if(in_atoms->size()<10){
  ...
  public: // ---- Intersects --------------------------------------------------------------------------------------------
    ...
    void make_intersects(){
      cnine::ftimer timer("TransferMap::make_intersects");

      PTENS_ASSRT(out_atoms->size()==n);
      PTENS_ASSRT(in_atoms->size()==m);
      for_each_edge([&](const int i, const int j, const float v){
	  Atoms in=(*in_atoms)[j];
	  Atoms out=(*out_atoms)[i];
	  Atoms common=out.intersect(in);
	  auto _in=in(common);
	  auto _out=out(common);
	  in_map->push_back(j,_in);
	  out_map->push_back(i,_out);
	  in_map->count1+=_in.size();
	  in_map->count2+=_in.size()*_in.size();
	  out_map->count1+=_out.size();
	  out_map->count2+=_out.size()*_out.size();
	    
	}, false);
      out_map->bmap=get_bmap();
    }
